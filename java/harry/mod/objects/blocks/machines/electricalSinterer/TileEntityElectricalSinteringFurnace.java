...
//helper import
import static harry.mod.util.helpers.EnumHelper.*;
...
public class TileEntityElectricalSinteringFurnace extends TileEntity implements IInventory, ISidedInventory, ITickable {
	...
	public void update() {
		...
		if (!this.world.isRemote) {
			...
			if (flag != this.isBurning()) {
				flag1 = true;
				BlockElectricalSinteringFurnace.setState(this.isBurning(), this.world, this.pos);
			}
		}
		if (flag1)
			this.markDirty();
	...
	private boolean canSmelt() {
		if (((ItemStack) this.inventory.get(0)).isEmpty() || ((ItemStack) this.inventory.get(1)).isEmpty())
			...
			ItemStack result = SinteringFurnaceRecipes.getInstance()
					.getSinteringResult((ItemStack) this.inventory.get(0), (ItemStack) this.inventory.get(1));
			if (result.isEmpty())
				return false;
			else {
	...
	private boolean canInsert(int index, ItemStack stack, EnumFacing direction) {
		return direction == EnumFacing.UP && isItemFuel(stack) ? isItemValidForSlot(index, stack)
				: (direction == EnumFacing.EAST && index == 1) || (direction == EnumFacing.WEST && index == 0);
	}
	...
	private EnumFacing getFacing(BlockPos pos) {
		...
		if (state.getProperties().containsKey(BlockElectricalSinteringFurnace.FACING)) {
			EnumFacing facing = state.getValue(BlockElectricalSinteringFurnace.FACING);
			return facing;
		}
		return null;
	}
