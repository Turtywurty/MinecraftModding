...
public class ContainerSinteringFurnace extends Container {
			...
			if (index == 3) {
				...
				if (!SinteringFurnaceRecipes.getInstance().getSinteringResult(stack1, slot1.getStack()).isEmpty()) {
					if (!this.mergeItemStack(stack1, 0, 2, false)) {
						return ItemStack.EMPTY;
					} else if (TileEntityElectricalSinteringFurnace.isItemFuel(stack1)) {
	...
	public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, EntityPlayer player) {
		...
		ItemStack[] inSlots = { this.inventorySlots.get(0).getStack(), this.inventorySlots.get(1).getStack(),
				this.inventorySlots.get(2).getStack() };
		int i = -1;
		for (ItemStack inSlot : inSlots) {
			i++;
			if (inSlot.getItem() == stack2.getItem() && inSlot.getMaxStackSize() >= inSlot.getCount() + stack2.getCount() && !inSlot.isEmpty()) {
				inSlot.setCount(inSlot.getCount() + stack2.getCount());
				this.inventorySlots.get(i).putStack(inSlot);
				break;
			} else if (inSlot.isEmpty() && !stack2.isEmpty()) {
				if(i == 2) {
					if(ForgeEventFactory.getItemBurnTime(stack2) > 0) {
						this.inventorySlots.get(i).putStack(stack2);
						break;
					}
				}
				else {
